...
class ContainercodeHelper
{
	
	/*
	 * Submenu for Joomla 1.6
	 */
	public static function addSubmenu($vName = 'firstchar')
	{
        
		JSubMenuHelper::addEntry(
			JText::_('Firstchar'),
			'index.php?option=com_containercode&view=firstchar',
			($vName == 'firstchar')
		);

		JSubMenuHelper::addEntry(
			JText::_('Secondchar'),
			'index.php?option=com_containercode&view=secondchar',
			($vName == 'secondchar')
		);

		JSubMenuHelper::addEntry(
			JText::_('Type'),
			'index.php?option=com_containercode&view=type',
			($vName == 'type')
		);

		JSubMenuHelper::addEntry(
			JText::_('Typegroup'),
			'index.php?option=com_containercode&view=typegroup',
			($vName == 'typegroup')
		);
	...
	public static function getExtensions() 
	{
		...
		$jv = new JVersion();
		$alt_libdir = ($jv->RELEASE < 1.6) ? JPATH_ADMINISTRATOR.DS.'components'.DS.'com_containercode' : null;
		JLoader::import('joomla.utilities.xmlelement', $alt_libdir);
		
		$xml = simplexml_load_file(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_containercode'.DS.'elements'.DS.'extensions.xml', 'JXMLElement');		        
 ...
 * @since		1.5
 */
abstract class JHtmlContainercode
{
	/**
	...
	{
		$extensions = ContainercodeHelper::getExtensions();
		$options = array();
		...
		foreach ($extensions as $extension) {   
			...
			$option->value = 'com_containercode.'.$extension->name;
			$options[] = $option;			
		}		
	...
	public static function categories($extension, $cat_id,$name="categories",$title="Select Category", $config = array('attributes'=>'class="inputbox"','filter.published' => array(0,1)))
	{
			...
			$query->from('#__containercode_categories AS a');
