...
/**
 * ContainercodeFirstchar Controller
 *
 * @package    Containercode
 * @subpackage Controllers
 */
class ContainercodeControllerFirstchar extends ContainercodeController
{
	/**
	 * Constructor
	 */
	protected $_viewname = 'firstchar'; 
	...
	public function publish() 
	{
		...
		$model = $this->getModel('firstchar');
		if (!$model->publish($cid, 1)) {
			echo "<script> alert('" . $model->getError(true) . "'); window.history.go(-1); </script>\n";
		}

		$this->setRedirect('index.php?option=com_containercode&view=firstchar');
	}
