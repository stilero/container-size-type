...
/**
 * ContainercodeTypegroup Controller
 *
 * @package    Containercode
 * @subpackage Controllers
 */
class ContainercodeControllerTypegroup extends ContainercodeController
{
	/**
	 * Constructor
	 */
	protected $_viewname = 'typegroup'; 
	...
	public function publish() 
	{
		...
		$model = $this->getModel('typegroup');
		if (!$model->publish($cid, 1)) {
			echo "<script> alert('" . $model->getError(true) . "'); window.history.go(-1); </script>\n";
		}

		$this->setRedirect('index.php?option=com_containercode&view=typegroup');
	}
